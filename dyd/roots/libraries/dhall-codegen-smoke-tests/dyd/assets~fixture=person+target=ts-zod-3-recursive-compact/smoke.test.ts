...
const invalid = Person.safeParse({
  ...ada,
  contact: { ...ada.contact, email: "not an email address" },
});

if (invalid.success) {
  throw new Error("expected invalid contact email to fail validation");
}
...
for (const phone_number of ["14155552671", "+0", "+1 4155552671", "+1234567890123456"]) {
  if (Person.safeParse({ ...ada, contact: { ...ada.contact, phone_number } }).success) {
    throw new Error(`expected invalid contact phone_number to fail validation: ${phone_number}`);
  }
}
